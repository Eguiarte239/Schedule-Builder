<div>
                                ...
                                <div class="px-2" wire:sortable.handle>
                                    <div class="flex flex-row justify-between">
                                        <div class="font-bold text-xl dark:text-white mb-2" >
                                            Project: {{ $task->phase->project->title }} <br> Phase: {{ $task->phase->title }} <br> Task: {{ $task->title }}
                                        </div> 
                                        <div>
                                            @can('assign-employee')
                                                @auth
    ...
    <x-jet-dialog-modal wire:model="openModal">
        ...
        <x-slot name="content">
            ...
            <div class="mb-4">
                <label for="assigned_to_phase" class="block mb-2 text-sm font-medium text-gray-900">
                    Phases
                </label>
                <select name="assigned_to_phase" id="assigned_to_phase" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" wire:model="assigned_to_phase">
                    <option value="" hidden selected></option>
                    @foreach ($phases as $phase)
                        <option value="{{ $phase->id }}">{{ $phase->title }}</option>
                    @endforeach
                </select>
                <x-jet-input-error for="assigned_to_phase"></x-jet-input-error>
                ...
                <x-jet-input-error for="assigned_to_task"></x-jet-input-error>
            </div>
            <div wire:ignore>
